...
import requests
from aes_help import  encrypt_data, decrypt_data
import zepp_helper as zeppHelper
...
def login(_user, password):
    ...
    登录
    """
    access_token, msg = zeppHelper.login_access_token("+86"+_user, password)
    if access_token is None:
        print("登录获取accessToken失败：%s" % msg)
        return None,None
    # print(f"device_id:{self.device_id} isPhone: {self.is_phone}")
    device_id = str(uuid.uuid4())
    login_token, app_token, user_id, msg = zeppHelper.grant_login_tokens(access_token, device_id, True)
    if login_token is None:
        print(f"登录提取的 access_token 无效：{msg}")
        return None,None
...
def main(_user, _passwd, _step):
    ...
    # print("login_token: "+str(login_token))
